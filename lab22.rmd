...
**Answer**: It doesn't differ a lot, and the slopes are significant. 
...
model <- lm_basic(warmth ~ 1 + speed + baseline, data = pf)
reg_table(model, level = 0.95)
```
...
**Answer**: Maybe because of overall skill level when it comes to using the computer. 
...
```{r}
model <- lm_basic(warmth ~ 1 + baseline + apprehension, data = pf)
reg_table(model, level = 0.95)
```
...
**Answer**: Both signs match across, even though one is positive and one is negative. 

Take the previous model and add the variables `speed`, `gender`, and
`country`, the latter lumped into 5 categories.

```{r}
model <- lm_basic(warmth ~ 1 + baseline + apprehension + speed + gender + fct_lump(country, 5), data = pf)
reg_table(model, level = 0.95)
```
...
**Answer**: The traits are all of statistical significance. 
