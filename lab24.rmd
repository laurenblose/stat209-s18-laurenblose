...
154 observations
The slopes differ, the yellow is much more extreme 
...
```{r}
crimes_n <- group_summarize(crimes, area_number, narcotics)
crimes_n <- left_join(crimes_n, ca, by = "area_number") 
crimes_n <- filter(crimes_n, narcotics == 1)

ggplot(crimes_n, aes(median_income, arrest_flag_mean)) + 
  geom_point() +
...
Excluding Archer Heights as the greatest outlier, the majority of areas all have high arrest means correlating with their median income.  
