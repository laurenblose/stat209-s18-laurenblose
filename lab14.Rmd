...
```{r}
ggplot(bird_level, aes(egg_mass_median, male_mass_median)) +
  geom_point() +
  scale_x_log10() +
  scale_y_log10()
```
...
```{r}
five_birds <- filter(bird_level, n >= 5)
five_birds
```
...
ggplot(bird_level, aes(egg_mass_median, male_mass_median)) +
  ...
  scale_x_log10() +
  scale_y_log10() +
  geom_point(color = "red", data = five_birds) + 
  xlab("Egg Mass Median") +
  ylab("Male Mass Median") +
  geom_text(aes(label = type, y = male_mass_median * 0.9), color = "red", size = 3, alpha = 1.0, data = five_birds)
```
