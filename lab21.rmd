...
**Answer**: The distribution is at its highest points between approx. 13 and 16. The bar plot works because it shows the distribution of scores across all those who have taken the test. 
...
**Answer**: The confidence interval is much bigger. The fewer data points, the less certain we are about the mean. 
...
**Answer**: It is not statistically significant. Not sure about stereotypes. 
...
```{r}
model <- lm_basic(warmth ~ 1 + fourties, data = agetf)
reg_table(model, level = 0.95)
```
...
**Answer**: Yes, and it is positive. Unsure about stereotypes. 
...
**Answer**: Yes because both values are positive. 

Fit a regression model that uses both gender and the baseline score to
predict your trait.

```{r}
model <- lm_basic(warmth ~ 1 + baseline + gender, data = pf)
reg_table(model, level = 0.95)
```

Describe the slope for the gender term in words:

**Answer**: Men tend to score 0.2357 lower. 
...
**Answer**: Model_pred increases as baseline increases. 
