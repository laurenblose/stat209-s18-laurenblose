...
**Answer**: There appears to be bias in the data in the way in which they picked. 
...
**Answer**: How much higher you expect the rating to be with chips included.
...
**Answer**: The range of answers included negative and positive values, not all negative or all positive. 
...
```{r}
model <- lm_basic(yelp ~ 1 + high_synergy, burrito)
reg_table(model, level = 0.95)
```
...
**Answer**: There is not enough evidence because the values are negative and positive, making it statistically insignificant. 

Now fit a regression predicting the overall score as a function of the
variable high_synergy.

```{r}
model <- lm_basic(overall ~ 1 + high_synergy, burrito)
reg_table(model, level = 0.95)
```
...
**Answer**: Yes because both values are positive, so it is statistically signficant. 
...
```{r}
model <- lm_basic(yelp ~ 1 + factor(synergy), burrito)
reg_table(model, level = 0.95)
```
...
**Answer**:(Intercept) - factor(synergy)4 
